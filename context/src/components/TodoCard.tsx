...
export default function TodoCard({ todo }: { todo: Todo }) {
  ...
  function handleCheckTodo(e: FormEvent) {
    e.preventDefault();

    updateTodo(todo._id, { checked: true });

    document.getElementById(`complete_modal_${todo._id}`)?.close();
    document.getElementById(`uncheck_modal_${todo._id}`)?.close();
  }

  function handleDeleteTodo(e: FormEvent) {
    e.preventDefault();

    deleteTodo(todo._id);

    document.getElementById(`delete_modal_${todo._id}`)?.close();
  }
          ...
          <dialog id={`delete_modal_${todo._id}`} className="modal">
            <div className="modal-box">
              ...
              <div>
                ...
                <div className="space-x-5 mt-5">
                  <button onClick={handleDeleteTodo} className="btn btn-ghost">
                    Sure
                  </button>
                  <button
                    onClick={() =>
